...
export const Playground = () => {
  const {
    state: { paginatedList, currentSize },
    actions: { setSize },
  } = Pagination.use<TableRow>();

  return (
    <div className="w-full space-y-6 overflow-x-auto rounded-lg border py-8 shadow-sm">
      <div className="flex justify-end">
        <Pagination.Select value={currentSize} onChange={setSize} />
      </div>
      <div className="mt-20">
        <Pagination.Control />
      </div>
      <table className="mt-6 w-full text-left text-sm">
        ...
        <tbody className="divide-y">
          {paginatedList.map((row, index) => (
            <tr
              key={row.id}
              className={`transition-colors duration-300 ${index === 0 ? "animate-success-row" : ""}`}
            >
