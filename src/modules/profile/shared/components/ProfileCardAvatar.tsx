...
export const ProfileCardAvatar = () => {
  ...
  const {
    bodyScrollLock: { shouldLockBodyScroll },
  } = AppLock.use();

  useEffect(() => {
    shouldLockBodyScroll(isAvatarSelectOpen);
  }, [isAvatarSelectOpen, shouldLockBodyScroll]);
  ...
  return (
    <>
      <div className="flex-center flex-col">
        ...
        <h1 className="mt-3 text-xl font-bold">{profile.current.hoTen}</h1>
      </div>
      {isAvatarSelectOpen && (
        <AvatarSelect
